...
const uploadFile = (f) => {
	return new Promise((resolve, reject) => {
		const projectId = "optimum-harbor-353509";
		const keyFileName = "../config/gcpStorageKey.json";
		const gc = new Storage({
			projectId,
			keyFileName,
		});
		const bucket = gc.bucket("wrinkle-bucket-too");
...
const addProduct = async (req, res, next) => {
	...
	try {
		...
		addItemImages = await pool.query(
			"INSERT INTO image (product_id, image_name, is_main) VALUES ($1, $2, $3), ($1, $4, $5), ($1, $6, $5)",
			[item_id, urlList[0], true, urlList[1], false, urlList[2]]
		);
	} catch (error) {
